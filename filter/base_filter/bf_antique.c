...
typedef struct XM_BFilter_antique
{
...
typedef struct XM_Filter_Opaque {
    XM_BFilter_antique *filter;
} XM_Filter_Opaque;

static void load_texture(XM_BFilter_antique *filter, int textures_num, int i, int w, int h, GLubyte *pixels)
{
...
static bool yuv420p_uploadTexture(XM_Filter_Opaque *opaque, SDL_VoutOverlay *overlay)
{
    ...
    XM_BFilter_antique *filter = opaque->filter;
...
static void setBitmap(XM_Filter_Opaque *opaque, XM_Bitmap xm_bmp[][EGL_BITMAP_NUMBER_MAX])
{
    ...
    XM_BFilter_antique *filter = opaque->filter;
    filter->xm_bmp = xm_bmp[V_BASE_EFFECTS_TYPE_ANTIQUE];
}
...
{
    return XM_BFilter_getFragmentShader_antique();
}
...
static void XM_BFilter_Antique_free(XM_BFilter_antique *filter)
{
    if (!filter)
        return;
    ...
    memset(filter, 0, sizeof(XM_BFilter_antique));
    free(filter);
}

static void XM_BFilter_Antique_freep(XM_BFilter_antique **filter)
{
    if (!filter || !*filter)
        return;

    XM_BFilter_Antique_free(*filter);
    *filter = NULL;
}
...
static void XM_BFilter_free(XM_Filter_Opaque *opaque)
{
    ...
    XM_BFilter_Antique_freep(&opaque->filter);
}
...
XM_Filter *XM_BFilter_Antique_create()
{
    ...
    opaque->filter = (XM_BFilter_antique *)calloc(1, sizeof(XM_BFilter_antique));
