...
typedef struct XM_BFilter_brannan
{
...
typedef struct XM_Filter_Opaque {
    XM_BFilter_brannan *filter;
} XM_Filter_Opaque;

static void load_texture(XM_BFilter_brannan *filter, int textures_num, int i, int w, int h, GLubyte *pixels)
{
...
static void shaderVariables_reset(XM_Filter_Opaque *opaque, XM_Filter_ShaderParameter *param)
{
    ...
    XM_BFilter_brannan *filter = opaque->filter;
...
static void setBitmap(XM_Filter_Opaque *opaque, XM_Bitmap xm_bmp[][EGL_BITMAP_NUMBER_MAX])
{
    ...
    XM_BFilter_brannan *filter = opaque->filter;
    filter->xm_bmp = xm_bmp[V_BASE_EFFECTS_TYPE_BRANNAN];
}
...
{
    return XM_BFilter_getFragmentShader_brannan();
}
...
static void XM_BFilter_Brannan_free(XM_BFilter_brannan *filter)
{
    if (!filter)
        return;

    //XM_TFilter_Fission_terminate(filter->opaque);
    memset(filter, 0, sizeof(XM_BFilter_brannan));
    free(filter);
}

static void XM_BFilter_Brannan_freep(XM_BFilter_brannan **filter)
{
    if (!filter || !*filter)
        return;

    XM_BFilter_Brannan_free(*filter);
    *filter = NULL;
}
...
static void XM_BFilter_free(XM_Filter_Opaque *opaque)
{
    ...
    XM_BFilter_Brannan_freep(&opaque->filter);
}
...
XM_Filter *XM_BFilter_Brannan_create()
{
    ...
    opaque->filter = (XM_BFilter_brannan *)calloc(1, sizeof(XM_BFilter_brannan));
