...
* base filter early-bird.
*/
#include "base_filter/base_renderer.h"
#include "base_filter/base_filter.h"

typedef struct XM_BFilter_earlybird
{
    XM_GLES2_Base_Renderer *renderer;
    XM_Bitmap *xm_bmp;
    GLuint sampler2D[5];
    GLuint textures[5];
} XM_BFilter_earlybird;

typedef struct XM_Filter_Opaque {
    XM_BFilter_earlybird *filter;
} XM_Filter_Opaque;

static void load_texture(XM_BFilter_earlybird *filter, int textures_num, int i, int w, int h, GLubyte *pixels)
{
...
static bool yuv420p_uploadTexture(XM_Filter_Opaque *opaque, SDL_VoutOverlay *overlay)
{
    ...
    XM_BFilter_earlybird *filter = opaque->filter;
    for(int i = 0; i < 5; i++)
    {
        glBindTexture(GL_TEXTURE_2D, filter->textures[i]);
...
static int yuv420p_use(XM_Filter_Opaque *opaque, int textures_num)
{
    ...
    if (0 == filter->textures[0])
        glGenTextures(5, filter->textures);
...
static void fsh_init(XM_Filter_Opaque *opaque)
{
    ...
    XM_BFilter_earlybird *filter = opaque->filter;
    filter->sampler2D[0] = glGetUniformLocation(filter->renderer->program, "inputImageTexture2");
    ...
    filter->sampler2D[3] = glGetUniformLocation(filter->renderer->program, "inputImageTexture5");
    filter->sampler2D[4] = glGetUniformLocation(filter->renderer->program, "inputImageTexture6");
}
...
static void setBitmap(XM_Filter_Opaque *opaque, XM_Bitmap xm_bmp[][EGL_BITMAP_NUMBER_MAX])
{
    ...
    XM_BFilter_earlybird *filter = opaque->filter;
    filter->xm_bmp = xm_bmp[V_BASE_EFFECTS_TYPE_EARLYBIRD];
}
...
{
    return XM_BFilter_getFragmentShader_earlybird();
}
...
static void XM_BFilter_EarlyBird_free(XM_BFilter_earlybird *filter)
{
    if (!filter)
        return;

    //XM_TFilter_Fission_terminate(filter->opaque);
    memset(filter, 0, sizeof(XM_BFilter_earlybird));
    free(filter);
}

static void XM_BFilter_EarlyBird_freep(XM_BFilter_earlybird **filter)
{
    if (!filter || !*filter)
        return;

    XM_BFilter_EarlyBird_free(*filter);
    *filter = NULL;
}
...
static void XM_BFilter_free(XM_Filter_Opaque *opaque)
{
    ...
    XM_BFilter_EarlyBird_freep(&opaque->filter);
}
...
XM_Filter *XM_BFilter_EarlyBird_create()
{
    ...
    opaque->filter = (XM_BFilter_earlybird *)calloc(1, sizeof(XM_BFilter_earlybird));
