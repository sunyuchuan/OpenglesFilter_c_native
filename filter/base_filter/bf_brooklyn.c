...
typedef struct XM_Filter_Opaque {
    XM_BFilter_brooklyn *filter;
} XM_Filter_Opaque;

static void load_texture(XM_BFilter_brooklyn *filter, int textures_num, int i, int w, int h, GLubyte *pixels)
{
...
static void shaderVariables_reset(XM_Filter_Opaque *opaque, XM_Filter_ShaderParameter *param)
{
    ...
    XM_BFilter_brooklyn *filter = opaque->filter;
...
static void setBitmap(XM_Filter_Opaque *opaque, XM_Bitmap xm_bmp[][EGL_BITMAP_NUMBER_MAX])
{
    ...
    XM_BFilter_brooklyn *filter = opaque->filter;
    filter->xm_bmp = xm_bmp[V_BASE_EFFECTS_TYPE_BROOKLYN];
}
...
{
    return XM_BFilter_getFragmentShader_brooklyn();
}
...
static void XM_BFilter_Brooklyn_free(XM_BFilter_brooklyn *filter)
{
    if (!filter)
        return;

    //XM_TFilter_Fission_terminate(filter->opaque);
    memset(filter, 0, sizeof(XM_BFilter_brooklyn));
    free(filter);
}

static void XM_BFilter_Brooklyn_freep(XM_BFilter_brooklyn **filter)
{
    if (!filter || !*filter)
        return;

    XM_BFilter_Brooklyn_free(*filter);
    *filter = NULL;
}
...
static void XM_BFilter_free(XM_Filter_Opaque *opaque)
{
    ...
    XM_BFilter_Brooklyn_freep(&opaque->filter);
}
...
XM_Filter *XM_BFilter_Brooklyn_create()
{
    ...
    opaque->filter = (XM_BFilter_brooklyn *)calloc(1, sizeof(XM_BFilter_brooklyn));
