...
typedef struct XM_BFilter_romance
{
...
typedef struct XM_Filter_Opaque {
    XM_BFilter_romance *filter;
} XM_Filter_Opaque;

static void load_texture(XM_BFilter_romance *filter, int textures_num, int i, int w, int h, GLubyte *pixels)
{
...
static bool yuv420p_uploadTexture(XM_Filter_Opaque *opaque, SDL_VoutOverlay *overlay)
{
    ...
    XM_BFilter_romance *filter = opaque->filter;
...
static void setBitmap(XM_Filter_Opaque *opaque, XM_Bitmap xm_bmp[][EGL_BITMAP_NUMBER_MAX])
{
    ...
    XM_BFilter_romance *filter = opaque->filter;
    filter->xm_bmp = xm_bmp[V_BASE_EFFECTS_TYPE_ROMANCE];
}
...
{
    return XM_BFilter_getFragmentShader_romance();
}
...
static void XM_BFilter_Romance_free(XM_BFilter_romance *filter)
{
    if (!filter)
        return;
    ...
    memset(filter, 0, sizeof(XM_BFilter_romance));
    free(filter);
}

static void XM_BFilter_Romance_freep(XM_BFilter_romance **filter)
{
    if (!filter || !*filter)
        return;

    XM_BFilter_Romance_free(*filter);
    *filter = NULL;
}
...
static void XM_BFilter_free(XM_Filter_Opaque *opaque)
{
    ...
    XM_BFilter_Romance_freep(&opaque->filter);
}
...
XM_Filter *XM_BFilter_Romance_create()
{
    ...
    opaque->filter = (XM_BFilter_romance *)calloc(1, sizeof(XM_BFilter_romance));
