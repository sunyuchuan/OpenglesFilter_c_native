...
typedef struct XM_BFilter_emerald
{
...
typedef struct XM_Filter_Opaque {
    XM_BFilter_emerald *filter;
} XM_Filter_Opaque;

static void load_texture(XM_BFilter_emerald *filter, int textures_num, int i, int w, int h, GLubyte *pixels)
{
...
static void pixels_malloc(XM_BFilter_emerald *filter)
{
    ...
    for (int j = 0; j < 256; j++){
        ...
        filter->pixels[(1 + (1024 + j * 4))] = arrayOfInt4[j];
...
static bool yuv420p_uploadTexture(XM_Filter_Opaque *opaque, SDL_VoutOverlay *overlay)
{
    ...
    XM_BFilter_emerald *filter = opaque->filter;
...
static void setBitmap(XM_Filter_Opaque *opaque, XM_Bitmap xm_bmp[][EGL_BITMAP_NUMBER_MAX])
{
    ...
    XM_BFilter_emerald *filter = opaque->filter;
    filter->xm_bmp = xm_bmp[V_BASE_EFFECTS_TYPE_EMERALD];
}
...
{
    return XM_BFilter_getFragmentShader_emerald();
}
...
static void XM_BFilter_Emerald_free(XM_BFilter_emerald *filter)
{
    if (!filter)
        return;
    ...
    memset(filter, 0, sizeof(XM_BFilter_emerald));
    free(filter);
}

static void XM_BFilter_Emerald_freep(XM_BFilter_emerald **filter)
{
    if (!filter || !*filter)
        return;

    XM_BFilter_Emerald_free(*filter);
    *filter = NULL;
}
...
static void XM_BFilter_free(XM_Filter_Opaque *opaque)
{
    ...
    XM_BFilter_Emerald_freep(&opaque->filter);
}
...
XM_Filter *XM_BFilter_Emerald_create()
{
    ...
    opaque->filter = (XM_BFilter_emerald *)calloc(1, sizeof(XM_BFilter_emerald));
