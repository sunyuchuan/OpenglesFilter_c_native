...
typedef struct XM_BFilter_warm
{
...
typedef struct XM_Filter_Opaque {
    XM_BFilter_warm *filter;
} XM_Filter_Opaque;

static void load_texture(XM_BFilter_warm *filter, int textures_num, int i, int w, int h, GLubyte *pixels)
{
...
static bool yuv420p_uploadTexture(XM_Filter_Opaque *opaque, SDL_VoutOverlay *overlay)
{
    ...
    XM_BFilter_warm *filter = opaque->filter;
...
static void fsh_init(XM_Filter_Opaque *opaque)
{
    ...
    filter->sampler2D[1] = glGetUniformLocation(filter->renderer->program, "greyFrame");
    filter->sampler2D[2] = glGetUniformLocation(filter->renderer->program, "layerImage");
}
...
static void setBitmap(XM_Filter_Opaque *opaque, XM_Bitmap xm_bmp[][EGL_BITMAP_NUMBER_MAX])
{
    ...
    XM_BFilter_warm *filter = opaque->filter;
    filter->xm_bmp = xm_bmp[V_BASE_EFFECTS_TYPE_WARM];
}
...
{
    return XM_BFilter_getFragmentShader_warm();
}
...
static void XM_BFilter_Warm_free(XM_BFilter_warm *filter)
{
    if (!filter)
        return;
    ...
    memset(filter, 0, sizeof(XM_BFilter_warm));
    free(filter);
}

static void XM_BFilter_Warm_freep(XM_BFilter_warm **filter)
{
    if (!filter || !*filter)
        return;

    XM_BFilter_Warm_free(*filter);
    *filter = NULL;
}
...
static void XM_BFilter_free(XM_Filter_Opaque *opaque)
{
    ...
    XM_BFilter_Warm_freep(&opaque->filter);
}
...
XM_Filter *XM_BFilter_Warm_create()
{
    ...
    opaque->filter = (XM_BFilter_warm *)calloc(1, sizeof(XM_BFilter_warm));
