...
* Create Fragment shaders of nostalgia effects
*/

#include "ijksdl/gles2_xm/internal.h"

static const char g_shader_nostalgia[] = XM_GLES_STRING(
precision highp float;
varying highp vec2 vv2_Texcoord;
uniform lowp sampler2D us2_Sampler;

uniform lowp sampler2D curve;
uniform lowp sampler2D curve2;
...
void main()
{
    ...
    vec4 textureColor = texture2D( us2_Sampler, vec2(xCoordinate, yCoordinate));
    ...
    lowp vec4 fragmentColor = texture2D(us2_Sampler, vec2(centerTextureCoordinate.x, centerTextureCoordinate.y)) * 0.2270270270;
    fragmentColor += texture2D(us2_Sampler, vec2(oneStepLeftTextureCoordinate.x, oneStepLeftTextureCoordinate.y)) * 0.3162162162;
    fragmentColor += texture2D(us2_Sampler, vec2(oneStepRightTextureCoordinate.x, oneStepRightTextureCoordinate.y)) * 0.3162162162;
    fragmentColor += texture2D(us2_Sampler, vec2(twoStepsLeftTextureCoordinate.x, twoStepsLeftTextureCoordinate.y)) * 0.0702702703;
    fragmentColor += texture2D(us2_Sampler, vec2(twoStepsRightTextureCoordinate.x, twoStepsRightTextureCoordinate.y)) * 0.0702702703;
