...
* Create Fragment shaders of sweets effects
*/

#include "ijksdl/gles2_xm/internal.h"

static const char g_shader_sweets[] = XM_GLES_STRING(
...
{
    vec4 color = texture2D(sampler, vv2_Texcoord) * 2.;

    color -= texture2D(sampler, vv2_Texcoord-vec2(texelWidthOffset, 0. )) * 0.25;
    color -= texture2D(sampler, vv2_Texcoord+vec2(texelWidthOffset, 0. )) * 0.25;
    color -= texture2D(sampler, vv2_Texcoord-vec2(0., texelHeightOffset)) * 0.25;
    color -= texture2D(sampler, vv2_Texcoord+vec2(0., texelHeightOffset)) * 0.25;
...
vec4 gaussianBlur(sampler2D sampler)
{
    ...
    color += texture2D(sampler,vv2_Texcoord)* 0.0443 ;

    step.x = 1.49583 * texelWidthOffset  * strength;
    step.y = 1.49583 * texelHeightOffset * strength;
    color += texture2D(sampler,vv2_Texcoord+vec2(step.x, 0.)) * 0.04321;
    color += texture2D(sampler,vv2_Texcoord-vec2(step.x, 0.)) * 0.04321;
    color += texture2D(sampler,vv2_Texcoord+vec2(0., step.y)) * 0.04321;
    color += texture2D(sampler,vv2_Texcoord-vec2(0., step.y)) * 0.04321;

    step.x = 2.4719250988753685 * texelWidthOffset	* strength;
    step.y = 2.4719250988753685 * texelHeightOffset * strength;
    color += texture2D(sampler,vv2_Texcoord+step) * 0.041795;
    color += texture2D(sampler,vv2_Texcoord-step) * 0.041795;
    color += texture2D(sampler,vv2_Texcoord+vec2(-step.x, step.y)) * 0.041795;
    color += texture2D(sampler,vv2_Texcoord+vec2( step.x,-step.y)) * 0.041795;

    step.x = 5.49583 * texelWidthOffset  * strength;
    step.y = 5.49583 * texelHeightOffset * strength;
    color += texture2D(sampler,vv2_Texcoord+vec2(step.x, 0.)) * 0.040425;
    color += texture2D(sampler,vv2_Texcoord-vec2(step.x, 0.)) * 0.040425;
    color += texture2D(sampler,vv2_Texcoord+vec2(0., step.y)) * 0.040425;
    color += texture2D(sampler,vv2_Texcoord-vec2(0., step.y)) * 0.040425;

    step.x = 5.300352223621558 * texelWidthOffset  * strength;
    step.y = 5.300352223621558 * texelHeightOffset * strength;
    color += texture2D(sampler,vv2_Texcoord+step) * 0.0391;
    color += texture2D(sampler,vv2_Texcoord-step) * 0.0391;
    color += texture2D(sampler,vv2_Texcoord+vec2(-step.x, step.y)) * 0.0391;
    color += texture2D(sampler,vv2_Texcoord+vec2( step.x,-step.y)) * 0.0391;

    step.x = 9.49583 * texelWidthOffset  * strength;
    step.y = 9.49583 * texelHeightOffset * strength;
    color += texture2D(sampler,vv2_Texcoord+vec2(step.x, 0.)) * 0.037815;
    color += texture2D(sampler,vv2_Texcoord-vec2(step.x, 0.)) * 0.037815;
    color += texture2D(sampler,vv2_Texcoord+vec2(0., step.y)) * 0.037815;
    color += texture2D(sampler,vv2_Texcoord-vec2(0., step.y)) * 0.037815;

    step.x = 8.128779348367749 * texelWidthOffset  * strength;
    step.y = 8.128779348367749 * texelHeightOffset * strength;
    color += texture2D(sampler,vv2_Texcoord+step) * 0.03658;
    color += texture2D(sampler,vv2_Texcoord-step) * 0.03658;
    color += texture2D(sampler,vv2_Texcoord+vec2(-step.x, step.y)) * 0.03658;
    color += texture2D(sampler,vv2_Texcoord+vec2( step.x,-step.y)) * 0.03658;
...
void main()
{
    ...
    if (lowPerformance == 1)
    {
        c1 = texture2D(us2_Sampler, vv2_Texcoord);
        c2 = texture2D(us2_Sampler, vv2_Texcoord);
    }
    else
    {
        c1 = sharpen(us2_Sampler);
        c2 = normal(c1, gaussianBlur(us2_Sampler), 0.8); // radius = 13. sharpen?? gaussian blur? ???? ??, ?? blending?? ??
    }
    ...
    c3 = normal(c3, overlay(c3, vec4(0.)), 1.-texture2D(samplerMask, vv2_Texcoord).r); // vignetting
