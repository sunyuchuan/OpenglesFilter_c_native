...
* Create Fragment shaders of freud effects
*/

#include "ijksdl/gles2_xm/internal.h"

static const char g_shader_freud[] = XM_GLES_STRING(
...
void main()
{
    ...
    vec4 color = texture2D(us2_Sampler, vv2_Texcoord);
...
//    color = NCColorControl(color, 0.6, -0.06, 0.75);
    ...
    // rand
    float x = vv2_Texcoord.x*inputImageTextureWidth/texture2Size;
    float y = vv2_Texcoord.y*inputImageTextureHeight/texture2Size;

    vec4 rd = texture2D(inputImageTexture2, vec2( fract(x), fract(y)));
//    vec4 rd = texture2D(inputImageTexture2, vv2_Texcoord);
//    float rand_number1 = PseudoRandom(vv2_Texcoord.xy);
//    float rand_number2 = PseudoRandom(vv2_Texcoord.yx);
