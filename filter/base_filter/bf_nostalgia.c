...
typedef struct XM_BFilter_nostalgia
{
    ...
    GLuint blurSize;
} XM_BFilter_nostalgia;

typedef struct XM_Filter_Opaque {
    XM_BFilter_nostalgia *filter;
} XM_Filter_Opaque;

static void load_texture(XM_BFilter_nostalgia *filter, int textures_num, int i, int w, int h, GLubyte *pixels)
{
...
static void shaderVariables_reset(XM_Filter_Opaque *opaque, XM_Filter_ShaderParameter *param)
{
    ...
    XM_BFilter_nostalgia *filter = opaque->filter;
    glUniform1f(filter->blurSize, 1.0f);
...
static int yuv420p_use(XM_Filter_Opaque *opaque, int textures_num)
{
    ...
    load_texture(filter, textures_num, 0, 256, 2, filter->pixels[0]);
    load_texture(filter, textures_num, 1, 256, 1, filter->pixels[1]);
    return 1;
}
...
static void fsh_init(XM_Filter_Opaque *opaque)
{
    ...
    filter->sampler2D[1] = glGetUniformLocation(filter->renderer->program, "curve2");
    ...
    filter->blurSize= glGetUniformLocation(filter->renderer->program, "blurSize");
}
...
static void setBitmap(XM_Filter_Opaque *opaque, XM_Bitmap xm_bmp[][EGL_BITMAP_NUMBER_MAX])
{
    ...
    XM_BFilter_nostalgia *filter = opaque->filter;
    filter->xm_bmp = xm_bmp[V_BASE_EFFECTS_TYPE_NOSTALGIA];
}
...
{
    return XM_BFilter_getFragmentShader_nostalgia();
}
...
static void XM_BFilter_reset(XM_Filter_Opaque *opaque)
{
    ...
    for (int i = 0; i < 2; i++) {
        if(filter->pixels[i])
        {
            free(filter->pixels[i]);
        }
        filter->pixels[i] = NULL;
    }
}

static void XM_BFilter_Nostalgia_free(XM_BFilter_nostalgia *filter)
{
    if (!filter)
        return;
    ...
    memset(filter, 0, sizeof(XM_BFilter_nostalgia));
    free(filter);
}

static void XM_BFilter_Nostalgia_freep(XM_BFilter_nostalgia **filter)
{
    if (!filter || !*filter)
        return;

    XM_BFilter_Nostalgia_free(*filter);
    *filter = NULL;
}
...
static void XM_BFilter_free(XM_Filter_Opaque *opaque)
{
    ...
    XM_BFilter_Nostalgia_freep(&opaque->filter);
}
...
XM_Filter *XM_BFilter_Nostalgia_create()
{
    ...
    opaque->filter = (XM_BFilter_nostalgia *)calloc(1, sizeof(XM_BFilter_nostalgia));
