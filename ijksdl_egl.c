...
static void IJKSDL_EGL_free(SDL_EGL_Opaque *opaque)
{
    if (!opaque || !opaque->ijk_egl)
        return;

    IJK_EGL_freep(&opaque->ijk_egl);
}

static void IJKSDL_EGL_terminate(SDL_EGL_Opaque *opaque)
{
    if (!opaque || !opaque->ijk_egl)
        return;

    IJK_EGL_terminate(opaque->ijk_egl);
}

static EGLBoolean IJKSDL_EGL_display(SDL_EGL_Opaque *opaque, EGLNativeWindowType window, SDL_VoutOverlay *overlay)
{
    if (!opaque || !opaque->ijk_egl)
        return EGL_FALSE;

    return IJK_EGL_display(opaque->ijk_egl, window, overlay);
}

SDL_EGL *IJKSDL_EGL_create()
{
    ...
    opaque->ijk_egl = IJK_EGL_create();

    sdl_egl->func_display  = IJKSDL_EGL_display;
    sdl_egl->func_terminate = IJKSDL_EGL_terminate;
    sdl_egl->func_free = IJKSDL_EGL_free;
