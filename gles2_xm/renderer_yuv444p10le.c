...
static GLboolean yuv444p10le_use(XM_GLES2_Renderer *renderer)
{
...
static GLsizei yuv444p10le_getBufferWidth(XM_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay)
{
    if (!overlay)
        return 0;
...
static GLboolean yuv444p10le_uploadTexture(XM_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay)
{
    if (!renderer || !overlay)
        return GL_FALSE;
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        glBindTexture(GL_TEXTURE_2D, renderer->plane_textures[i]);  XM_GLES2_checkError_TRACE("glBindTexture");
        ...
        glTexImage2D(GL_TEXTURE_2D,
                     ...
                     GL_LUMINANCE_ALPHA,
                     GL_UNSIGNED_BYTE,
                     pixels[plane]);    XM_GLES2_checkError_TRACE("glTexImage2D");
    }
...
XM_GLES2_Renderer *XM_GLES2_Renderer_create_yuv444p10le(SDL_VoutOverlay *overlay)
{
    ...
    const char *fsh = XM_GLES2_getFragmentShader_yuv444p10le();
